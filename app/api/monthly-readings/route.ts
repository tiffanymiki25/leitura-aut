...
export async function POST(request:Request) {
  ...
  const {month,bookIds} = await request.json();
  ...
  const reading = await prisma.monthlyReading.upsert({where:{month:date},update:{startDate:null,books:{deleteMany:{},create:books.map(book=>({bookId:book.id}))}},create:{month:date,books:{create:books.map(book=>({bookId:book.id}))}},include:{books:{include:{book:true}}}});
