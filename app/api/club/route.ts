...
export async function GET() {
  ...
  const [members,books,monthlyReadings,currentReading,ownStatuses,allStatuses]=await Promise.all([
  ...
  return NextResponse.json({club:{...club,currentBook:currentReading?.books[0]?.book||null},members,books,statuses,monthlyReadings,user,profileStats,ownStatuses,readerRanking,bookRanking,readingReport});
}
